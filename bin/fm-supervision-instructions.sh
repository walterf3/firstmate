...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    pi|pi-signed)
      printf '%s%s%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the Pi tool fm_watch_arm_pi, or restart the selected Pi executable with -e ' "$pi_turnend_ext" ' -e ' "$pi_ext" ' if the extensions are not loaded.'
      ;;
    omp)
      printf '%s%s%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the OMP-embedded Pi tool fm_watch_arm_pi, or restart omp with -e ' "$pi_turnend_ext" ' -e ' "$pi_ext" ' if the extensions are not loaded.'
      ;;
    opencode)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    pi|pi-signed)
      ...
      ;;
    omp)
      printf '%s\n' '- Ordinary wake: the OMP-embedded Pi extension already owns watcher continuity; do not arm another cycle.'
      ;;
    opencode)
