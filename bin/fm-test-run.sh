...
family_for_basename() {
  case "$1" in
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-integrate-direct.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-gate-refuse*|bin/fm-lock*|bin/fm-quota-axi-lib.sh)
      printf '%s\n' session-bootstrap
      ;;
    bin/fm-pr-*|bin/fm-merge-local.sh|bin/fm-integrate-direct.sh|bin/fm-teardown.sh|bin/fm-review-diff.sh|\
