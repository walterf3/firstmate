...
- The receipt is never overwritten and the meta never gains a second `landed=` line, and the `already-landed` refusal is the single diagnostic that names the receipt and its SHA on a rerun.
