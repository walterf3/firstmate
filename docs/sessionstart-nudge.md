...
| OMP | The embedded Pi extension handles `session_start` reasons `startup`, `new`, and `resume`, then injects the wrapper output with `pi.sendMessage`. | The custom message reaches OMP model context without racing an initial positional prompt. |
