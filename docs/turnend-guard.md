...
- OMP uses the same `.pi/extensions/fm-primary-turnend-guard.ts` through its embedded Pi surface, selecting `agent_end` for its logical-run boundary and the same bounded follow-up behavior.
